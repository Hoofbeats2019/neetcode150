...
| [Minimum Interval to Include Each Query](solutions/minimum_interval_to_include_each_query.py) | Intervals | Solved | Sweep sorted queries while a min-heap tracks the shortest active intervals | O((n + q) log(n + q)) | O(n + q) |
...
- Sort queries with their original indexes so results can be restored to input order.
- Add every interval that has started to a min-heap ordered by its inclusive length.
- Remove a heap entry only when its end is before the current query; the remaining top is the shortest valid interval.
