...
| [Subtree of Another Tree](solutions/subtree_of_another_tree.py) | Trees | Solved | DFS each candidate and recursively compare both trees | O(n x m) | O(h + k) |
...
- Use DFS to consider every node in the main tree as the possible root of the requested subtree.
- At each candidate, recursively compare corresponding values and child structures in both trees.
- Two missing nodes match, while exactly one missing node means the subtree structures differ.
- If one candidate does not match, continue searching both branches of the main tree.
- In the worst case, comparing a subtree of m nodes at each of n candidates takes O(n x m) time.
